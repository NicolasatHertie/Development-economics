...
SH.XPD.OOPC.TO.ZS
wbdata <- c('SH.XPD.TOTL.ZS', 'SP.POP.TOTL')
...
dataset2 <- plyr::rename(dataset2, c("SH.XPD.TOTL.ZS" = "HCE"))
...
sum(is.na(dataset2$HCE))

dataset2 <- dataset2[!is.na(dataset2$HCE),]
hist(dataset2$HCE)

dataset2$lHCE <- log(dataset2$HCE)
hist(dataset2$lHCE)

shapiro.test(dataset2$HCE)
shapiro.test(dataset2$lHCE)
